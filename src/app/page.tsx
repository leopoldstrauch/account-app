"use client";

import { useEffect, useState } from 'react';
import EntryForm from '@/components/EntryForm';
import EntryList from '@/components/EntryList';
import { Account } from '../core/types/Account';
import { Entry } from '../core/types/Entry';
import Button from '@/components/Button';

type EntryInput = Omit<Entry, 'id' | 'status'> & { id?: number };

export default function Home() {
  const [accounts, setAccounts] = useState<Account[]>([]);
  const [entries, setEntries] = useState<Entry[]>([]);
  const [error, setError] = useState('');
  const [editingEntry, setEditingEntry] = useState<Entry | null>(null);

  useEffect(() => {
    fetchAccounts();
    fetchEntries();
  }, []);

  const fetchAccounts = async () => {
    const response = await fetch('/api/accounts');
    const data = await response.json();
    setAccounts(data);
  };

  const fetchEntries = async () => {
    const response = await fetch('/api/entries');
    const data = await response.json();
    setEntries(data);
  };

  const handleSaveEntry = async (entry: EntryInput) => {
    setError('');

    let response;
    if (entry.id) {
      response = await fetch('/api/entries', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(entry),
      });
    } else {
      response = await fetch('/api/entries', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify(entry),
      });
    }

    const newEntry = await response.json();
    fetchEntries();
    setEditingEntry(null);
  };

  const handleEditEntry = (entry: Entry) => {
    setEditingEntry(entry);
  };

  const handleDeleteEntry = async (id: number) => {
    if (!confirm('Sind Sie sicher, dass Sie diesen Buchungssatz löschen möchten?')) {
      return;
    }

    await fetch('/api/entries', {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id }),
    });

    fetchEntries();
  };

  const handleReverseEntry = async (id: number) => {
    const response = await fetch('/api/entries/reverse', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id }),
    });

    const newEntry = await response.json();
    fetchEntries();
  };

  const postEntries = async () => {
    const res = await fetch('/api/entries', {
      method: 'PATCH',
    });
    const data = await res.json();
    console.log(data.message);
    fetchEntries();
  };

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Buchungssätze Verwaltung</h1>
      {error && <p className="text-red-500">{error}</p>}
      <EntryForm
        accounts={accounts}
        editingEntry={editingEntry}
        onSave={handleSaveEntry}
        error={error}
        setError={setError}
      />
      <Button onClick={postEntries} className="bg-green-600 hover:bg-green-700">
        Buchungssätze buchen
      </Button>
      <h2 className="text-xl font-semibold mb-2">Buchungssätze</h2>
      <EntryList
        entries={entries}
        accounts={accounts}
        onEdit={handleEditEntry}
        onDelete={handleDeleteEntry}
        onReverse={handleReverseEntry}
      />
    </div>
  );
}
